...
**Initial date:** 2026-05-08 (CPU)  
**GPU latency update:** 2026-05-18 (NVIDIA L4)
...
## GPU Latency (2026-05-18 update)

The service was migrated from 2-vCPU CPU inference to an NVIDIA L4 GPU on
2026-05-18. Warm-instance latency with `n_beams=10`:

| Molecule | MW (Da) | GPU (L4) | CPU (2 vCPU) | Change |
|---|---|---|---|---|
| Aspirin | 180 | **2.7 s** | ~8–12 s | ~4× faster |
| Ibuprofen | 206 | **3.1 s** | ~10–15 s | ~4× faster |
| Orforglipron | 881 | **25.5 s** | timeout (>90 s) | now completes |

Orforglipron previously timed out on the CPU deployment; the GPU completes
beam search in ~25 s. Top log-likelihood on GPU (−22.9) matches the CPU result
from the earlier test, confirming model determinism across hardware.
...
def predict_retrosynthesis(smiles: str, n_beams: int = 10) -> dict:
    response = requests.post(
        f"{SERVICE_URL}/retrosynthesis/predict",
        json={"smiles": smiles, "n_beams": n_beams},
        timeout=(10, 120),  # (connect, read); GPU warm ~3–26 s, cold-start ~90 s
    )
...
result = predict_retrosynthesis(orforglipron_smiles, n_beams=10)
for i, p in enumerate(result["predictions"], 1):
    print(f"Beam {i} (ll={p['log_likelihood']:.3f}): {p['reactants_smiles'][:80]}...")
```

**Output** (GPU, 10 beams, 25.5 s warm):
```
Beam  1 (ll=-22.907): C=CC1=C([C@@]2(C3=NOC(=O)N3)C[C@@H]2C)N2C(=O)C3=CC4=CC(=C[C@@H]5CC(C)(C)...
Beam  2 (ll=-23.274): C=CC1=C([C@@]2(C3=NOC(=O)N3)C[C@@H]2C)N2C(=O)C3=CC4=CC(=C[C@@H]5CC(C)(C)...
Beam  3 (ll=-23.325): C=CC1=C([C@@]2(C3=NOC(=O)N3)C[C@@H]2C)N2C(=O)C3=CC4=CC(=C[C@@H]5CC(C)(C)...
Beam  4 (ll=-23.371): C=CC1=C([C@@]2(C3=NOC(=O)N3)C[C@@H]2C)N2C(=O)C3=CC4=CC(=C[C@@H]5CC(C)(C)...
Beam  5 (ll=-23.714): C=CC1=C([C@@]2(C3=NOC(=O)N3)C[C@@H]2C)N2C(=O)C3=CC4=CC(=C[C@@H]5CC(C)(C)...
Beam  6 (ll=-23.771): C=CC1=C([C@@]2(C3=NOC(=O)N3)C[C@@H]2C)N2C(=O)C3=CC4=CC(=C[C@@H]5CC(C)(C)...
Beam  7 (ll=-23.782): C=CC1=C([C@@]2(C3=NOC(=O)N3)C[C@@H]2C)N2C(=O)C3=CC4=CC(=C[C@@H]5CC(C)(C)...
Beam  8 (ll=-24.084): C=CC1=C([C@@]2(C3=NOC(=O)N3)C[C@@H]2C)N2C(=O)C3=CC4=CC(=C[C@@H]5CC(C)(C)...
Beam  9 (ll=-24.194): C=CC1=C([C@@]2(C3=NOC(=O)N3)C[C@@H]2C)N2C(=O)C3=CC4=CC(=C[C@@H]5CC(C)(C)...
Beam 10 (ll=-24.449): C=CC1=C([C@@]2(C3=NOC(=O)N3)C[C@@H]2C)N2C(=O)C3=CC4=CC(=C[C@@H]5CC(C)(C)...
```

*Note: this molecule previously timed out on the CPU deployment (>90 s). GPU completes in ~25.5 s.*

**Assessment:** Poor, as expected. Several failure modes are evident:

1. **Very low confidence** — log-likelihoods of −22.9 to −24.4, compared to −0.7 to −1.0 for simple drugs. The model assigns near-zero probability to any single-step route.

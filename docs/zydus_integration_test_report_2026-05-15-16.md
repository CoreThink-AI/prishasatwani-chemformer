...
## Latency optimization 2026-05-16
```bash
--gpu-type nvidia-l4 \
--memory 16Gi \
--cpu 4 \
...
### Latency (warm NVIDIA L4 GPU)
┌──────────────┬────────┬─────────────┬─────────────────┐
│   Molecule   │   MW   │ GPU latency │  CPU baseline   │
├──────────────┼────────┼─────────────┼─────────────────┤
│ Aspirin      │ 180 Da │ 2.7 s       │ ~8–12 s         │
├──────────────┼────────┼─────────────┼─────────────────┤
│ Ibuprofen    │ 206 Da │ 3.1 s       │ ~10–15 s        │
├──────────────┼────────┼─────────────┼─────────────────┤
│ Orforglipron │ 881 Da │ 25.5 s      │ timeout (>90 s) │
└──────────────┴────────┴─────────────┴─────────────────┘

Orforglipron now completes in ~26 s — previously it timed out entirely on the 2-vCPU CPU deployment. Predictions (log-likelihood −22.9 to −24.4) match the values in the prior report.

GPU warm: Aspirin ~3 s, Orforglipron ~26 s
GPU cold-start: add ~60 s for L4 cache initialisation

### Deployment bug fixes
1. "GLIBC_2.33 not found": pinned builder to python:3.8-slim-bullseye (both builder and runtime now GLIBC 2.31)
2. Vocab 404 — corrected path to gs://biochem-db-by-hobs/chemformer/retrosynthesis/bart_vocab.json
3. GPU quota: added `--max-instances 3 --no-gpu-zonal-redundancy` to both CI/CD files (GitLab and GitHub)
4. `timeout=N` can be defeated by HTTP/2 PING keepalive frames (they reset the socket timer without delivering data). Always use a tuple:
`timeout=(10, 120)  # (connect_timeout_s, read_timeout_s)` 

- SEE docs/CLAUDE-retrosynthesis-pathway.md
- Cloud Run settings updated: GPU L4, 16 GiB, 4 CPU, max-instances 3
- Redeploy workflow now submit gcloud build requests + full GPU flags
- New Latency section with the measured numbers table
- Known Limitations: replaced the "MW > 600 Da timeout" entry with a GPU cold-start note (~60 s)
- HTTP timeout recommendation updated to (10, 120)

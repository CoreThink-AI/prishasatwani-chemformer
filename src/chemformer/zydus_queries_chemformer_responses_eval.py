...
def fetch_compound(cid: int) -> dict:
    ...
    vendors = _get(f"{PUBCHEM_BASE}/compound/cid/{cid}/xrefs/SourceName/JSON").json()
    vendor_count = len(vendors.get("InformationList", {}).get("Information", [{}])[0].get("SourceName", [])) if rv.ok else 0

    bioassays = _get(f"{PUBCHEM_BASE}/compound/cid/{cid}/assaysummary/JSON").json()
    bioassay_count = len(bioassays.get("Table", {}).get("Row", [])) if rb.ok else 0
